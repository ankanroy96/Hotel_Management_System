...
<body>
  <div class="container-flud">
    ...
    $sql = "Select country from employee where id=$id";
    $result = mysqli_query($conn, $sql) or die("Query failed");

    if(mysqli_num_rows($result) >0) {
      $row = mysqli_fetch_assoc($result);
     ?>
     <form action="countryfinal.php?id=<?php echo $id?>" method="post">
     <div class="row">
       <div class="col-sm-4 m-auto">
         <div class="form-group">
         <label>Country:</label>
         <input type="text" id="fname" class="form-control-plaintext" value="<?php echo $row['country'];?>" readonly>
       </div>
       </div>
     </div>
    <div class="row">
      <div class="col-sm-4 m-auto">
        ...
        <input type="text" name="newcountry" id="newcountry" class="form-control" placeholder="Enter New Country" required>
      </div>
      </div>
    </div>
