...
if(isset($_POST['room_type']) && isset($_POST['size_type'])){
  $roomType=$_POST['room_type'];
  $sizeType=$_POST['size_type'];
  if($roomType=="all" && $sizeType=="all"){
    header("Location: http://localhost/hotel_management_system/admin/reception/availableAllRooms.php");
  }
  else if($roomType!="all" && $sizeType=="all"){
    unset($_POST['size_type']);
  }
  else if($roomType=="all" && $sizeType!="all"){
    unset($_POST['room_type']);
  }
}
else if(isset($_POST['room_type']) && !isset($_POST['size_type'])){
  $roomType=$_POST['room_type'];
  if($roomType=="all"){
    header("Location: http://localhost/hotel_management_system/admin/reception/availableAllRooms.php");
  }
...
<body>
  <?php
    ...
    if((isset($_POST['room_type']) && isset($_POST['size_type']))){
    $sql = "Select * from rooms where room_type='$roomType' and size_type='$sizeType' and status=1";
    }
    else if((isset($_POST['room_type']) && !isset($_POST['size_type']))){
    $sql = "Select * from rooms where room_type='$roomType' and status=1";
    }
    else if((!isset($_POST['room_type']) && isset($_POST['size_type']))){
    $sql = "Select * from rooms where size_type='$sizeType' and status=1";
    }
