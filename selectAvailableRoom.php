    ...
    <div class="row">
      <div class="offset-3 col-6">
        <table class="table table-striped">
          ...
          //database connection
          if($size=="all"){
          $sql = "Select * from rooms where room_type='$type' and status=1";
        }
        else{
          $sql = "Select * from rooms where room_type='$type' and status=1 and size_type='$size'";
        }
          ...
          if(mysqli_num_rows($result) >0) {
              ...
              if(mysqli_num_rows($result2) == 0){
                ...
                <?php } ?>
                  <td class="text-center">
                    <a href="bookedRooms.php?room=<?php echo $room;?>&price=<?php echo $row['fprice']?>&loc=2">Book Now</a>
                  </td>
                </tr>
           ...
           <?php
           $sql3 = "Select * from cart where email='$email' and cindate='$cin' and coutdate='$cout'";
           ...
           if(mysqli_num_rows($result3) >0) {
             ...
             while($row5 = mysqli_fetch_assoc($result3)){
               $ro=$row5['room'];
               $sql30 = "Select * from rooms where id=$ro";
               $result30 = mysqli_query($conn, $sql30) or die("Query failed2");
               if(mysqli_num_rows($result30) >0) {
                 $row30 = mysqli_fetch_assoc($result30);
               }
           ?>
           <tr>
             <td class="text-center"><?php echo $row5['room'];?></td>
             <td class="text-center"><a href="viewRooms.php?room=<?php echo $room;?>">View Room</a></td>
             <td class="text-center"><?php echo $row30['size_type'];?></td>
             <td class="text-center"><?php echo $row30['adult'];?></td>
             <td class="text-center"><?php echo $row30['child'];?></td>
             <?php if($row30['price']==$row30['fprice']){ ?>
             <td class="text-center"><?php echo $row30['price'];?></td>
           <?php }
           else {
             ?>
             <td class="text-center"><s><?php echo $row30['price'].'</s><br>'; echo $row30['fprice'];?></td>
           <?php } ?>
             <td class="text-center">
               <a href="cancelBooking.php?room=<?php echo $row5['room'];?>&id=2" style="color: red;">Cancel</a>
             </td>
           </tr>
